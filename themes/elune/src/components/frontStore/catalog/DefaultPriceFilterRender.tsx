...
/**
 * Core's `DefaultPriceFilterRender`, logic verbatim, plus the two accessible
 * names DESIGN.md requires of every field: the group (`aria-label`) and each
 * range handle (`getAriaLabel`). Core passed no label, so its two
 * `input[type=range]` elements were announced unnamed.
 */
      ...
      <div className="price__filter border-b border-border pb-8 mb-8">
        <div className="price__slider mb-4">
          <Slider
            ...
            aria-label={_('Price range')}
            getAriaLabel={(index) =>
              index === 0 ? _('Minimum price') : _('Maximum price')
            }
          />
        </div>
