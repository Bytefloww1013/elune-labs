...
- **TARGET_VERSION:** `v4`        <-- [Set to: v2, v3, v4, etc.]
- **SOURCE_VERSION:** `v3` 
...
- **Target Element:** social-proof section (above footer) - with --night background color
- **Instruction:** Using the img assets located at: `docs/refs/assets/imgs/` replace the current content in the referenced element with the following:
    - 3 sperate testamonial cards that will appear as if they are actual testamonials for the products sold on the site.
    - The image assets are located at: `docs/refs/assets/imgs/` and are named as follows:
        - `td-1.jpg`
        - `td-2.jpg`
        - `td-3.jpg`
      - Be sure to talk about the high quality of the product, and the fast shipping. And how seemless the order process was.  Use unique tone and language in each testamonial quote.  
- **Styling Rules:** 
  - Ensure the cards are spaced corretly, styling is used for the quotes, and can be visually represented as if they are actual testamonials for the products sold on the site. You are able to resize the images if required.  When using a person's name only show first name and last initial.  When using a city and state only show the city and state abbreviation.
