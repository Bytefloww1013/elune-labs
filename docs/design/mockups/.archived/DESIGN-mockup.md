# Design Brief — Elune Labs Landing Redesign ("Lunar Plates")

**Status:** proposal for review. `DESIGN.md` (the shipped Warm Paper system) is untouched; on approval this
...
**Prototype:** `docs/design/mockups/elune-landing-mockup.html` (single self-contained file, no JS, no network).
...
sits natively beside the type and the CTA instead of fighting a saturated hue. On the one dark band the
white asset twin is used.
...
products → footer. One region is added — the specification record, pending D2 — and it is a region inside
the canon order, not a reordering. No dossier, archive or specimen register returns.

**Thesis.** A storefront that behaves like a catalogue of plates: the object and its record share the first
viewport at the same scale, so trust is earned by what is written down, not by atmosphere. It refuses the
category's hype terminals and equally refuses lab-cold institution.
...
| `--night` | `#16233C` | `hsl(219 46% 16%)` | **The only chromatic authority:** text, button fills, the one dark band, caret, focus ring | 14.75:1 on snow |
...
| `--night-rule` | `#33415F` | `hsl(221 30% 29%)` | Rules on the ink band | — |
...
1.04 / 0.96 · ink band 0.86 / 1.14. Desktop 12-col thinking, tablet 8, mobile 4, collapsing to one column
...
centred, not centred in the leftover space. Official `website_header_logo_transparent.png` left at 38px
height; Home · New releases · Shop (native `<details>` disclosure holding the five live categories, each with
...
white sheet (32px radius, `--ash` edge) holding the produced photograph cropped to portrait on the left and
...
**Omission state (every plate and the record table).** `productSpecs.ts` is the only source and it omits
...
therefore BPC-157-5MG's own shape, not a fixed template, and the ink-band table is the same record at full
length. Values are the record's exact strings — storage is "Store cool and dry, away from direct light", not
...
**The record (one `--night` band).** Left: heading, one paragraph, a beam pill CTA. Right: the real
specification table for the hero compound, hairline-ruled in `--night-rule`, values mono and white, purity
row in beam, sourced from `productSpecs.ts` in its own field order. A cratered-disc motif (authored SVG,
16% white, bleeding off the right) is the only ornament in the system. The RUO line and the "specification,
not a test result" caveat close the band.

**Social proof** (the prompt's Social Proof component, answered
with documentation proof rather than people proof). None is fabricated — no testimonials, ratings, press marks, customer counts or badges of
trust exist in this repo and none may be invented. The specification record does that job; until verified
reviews exist, the section is *documentation proof* (what a buyer can check before ordering) or it is absent.
If reviews ever ship: source, date and moderation policy shown, no efficacy or human-use claims.
...
- **D2 — the one dark band.** Light-only is honoured as *the theme*; a single `--night` region carries the
  specification. Say the word and it becomes `--mist` at identical structure.
...
10. No fabricated social proof, metrics, partner logos or press marks.
...
- Breakpoints 1080 (cards 2-up, strip 2-up) · 900 (hero and ink band stack, links collapse, trust 2-up) ·
  640 (everything single column, trust full measure, record rows stack label-over-value). Verified: zero
  horizontal overflow at 390px; all four product cards equal height at 1440px.
...
6. `hero-photo.webp` keeps its sidecar provenance; the portrait crop is CSS (`object-fit: cover` +
   `translate(16px,-2px) scale(1.36)`), so no new raster is produced.
