# Design Brief — Elune Labs Landing Redesign ("Lunar Plates") — v5

**Status:** proposal for review, **v5 working copy**. `DESIGN.md` (the shipped Warm Paper system) is untouched; on approval this
brief promotes to `DESIGN.md` + `.impeccable/design.json`, and the prototype becomes the build blueprint.
The base brief (`DESIGN-mockup.md`) remains the full specification; this v5 file carries the v1 delta in §0 and the v2–v5 deltas in the Version Changelog at the end.
**Surface:** `themes/elune/src/pages/homepage/Elune.tsx` · **Mode:** Persuade, constrained by storefront trust.
**Prototype:** `docs/design/mockups/elune-landing-mockup-v5.html` (single self-contained file, no JS, no network).
...
## Version Changelog: v5

### v5.0 Ink band padding tightened, card backing removed
Owner directive: trim the empty space above "From recent orders." and below the transparency foot so the band hugs its content, and remove the dark panel backing behind the testimonial text — the quote should sit on the band ground, with styling carried by type and hairlines rather than a highlighted card.

- **Band padding:** `.ink` now declares its own `padding-block: clamp(36px, 4.5vw, 56px) clamp(32px, 4vw, 48px)`, overriding `.section`'s generic `clamp(56px, 7vw, 96px)` on this band only. Top gap tightens by roughly 40% and the bottom gap tightens further, so the band ends just below the caveat line instead of carrying the standard section rhythm into the footer.
- **Card chrome stripped:** `.tcard` loses `background: var(--night-panel)`, `border: 1px solid var(--night-rule)`, its card padding and its radius. What remains is `padding: 20px 0 0` and a `border-top: 1px solid var(--night-rule)` — each column reads as an editorial entry under a hairline rule, with the quote set directly on the `--night` ground in snow. The photo frame (`.tshot`) keeps its own hairline and radius, since the image itself is the bounded element.
- **Rationale:** the panel-backed card was doing visual work the band ground already does — the dark card read as a second dark surface stacked on the first, and the 24px card padding added a full extra rhythm around text that already had its own spacing. Stripping the panel flattens the hierarchy back to one surface, and the hairline rule above each entry supplies the separation the panel border used to fake.
- **Contrast, recomputed:** snow on `--night` 15.23:1 (quote, name — up from 13.23:1 against the panel); `--night-muted` on `--night` 9.90:1 (meta, transparency foot). WCAG AA margins widen, not narrow.
- **Token consequence:** `--night-panel` has no remaining consumer in this file. It is left declared for one version so the v4 → v5 diff stays minimal; it can be removed on the next pass if nothing else adopts it. No new colour, no new type step, no new motion.

### v5.1 Quote becomes chat-quote
Owner directive: style the testimonial quote as a modern chat/message bubble — tinted ground, left accent bar, serif italic.

- **Selector:** `.tcard blockquote` → `.proof-grid blockquote`. Same specificity class (0,1,1 → 0,1,1); the rename ties the rule to the grid (the layout context) instead of the card (the chrome the quote no longer lives inside).
- **Background:** `color-mix(in srgb, var(--night) 85%, #fff 15%)` — a single-step elevation of the band ground (resolves to `#394459`). Derives from `--night`, so a future band re-tint propagates; no new token declared. Replaces the flat-on-band treatment from v5.0, which the owner read as unstyled.
- **Accent:** `border-left: 3px solid var(--beam)` with `border-radius: 0 6px 6px 0`. This spends the beam on a 3px bar per quote — the first non-badge beam surface on the page, and the owner's explicit ask. Beam on the tint is 7.73:1, well past the 3:1 floor for non-text UI.
- **Typography:** `Georgia, "Times New Roman", serif`, italic, 1.05rem on the existing 1.55 line-height. System serif on purpose: no new webfont payload for three short quotes, and the italic serif against the sans body gives the quote its own voice without a display face. Slight size bump (16 → 16.8px at the 17px root) separates quote from meta.
- **Internal spacing:** `padding: .75rem 1rem`, so text sits 12px off the accent bar and 16px off the other three sides.
- **Card cleanup:** `.tcard` loses its v5.0 `border-top` hairline and `padding-top` — the quote bubble now provides the visual top of each entry, and the hairline read as a redundant stacked rule. `.tcard` is now a plain flex column with no chrome of its own.
- **Contrast, recomputed:** snow on `#394459` 9.51:1 (quote text), `--night-muted` on the band still 9.90:1 (meta, foot). All pairs clear AA.

### v5.2 Testimonials become cards
Owner directive: wrap each testimonial in a polished card; author block (avatar, name, city/state, date) grouped; then a follow-up: remove the circle avatars.

- **Card:** `.tcard` regains chrome as a real surface — `padding: 1.5rem`, `background: var(--night-surface)`, `border: 1px solid var(--night-rule)`, `border-radius: var(--r-card)`, `box-shadow: 0 8px 24px rgba(0,0,0,.22)` (offset + blur, no halo, the page's first shadow on the dark band — the light side stays flat per the token system). The grid already stretches, so cards stay equal height.
- **New token:** `--night-surface: color-mix(in srgb, var(--night) 85%, #fff 15%)` — the v5.1 quote tint promoted to a token (two consumers would have existed: card and quote). Declared beside `--night-panel`, which it effectively supersedes; `--night-panel` remains only as a historical alias and can be deleted next pass.
- **Quote:** blockquote background returns to `transparent` — the card supplies the surface; a second tint inside the first read as double-boxing. The beam left bar, serif italic, and padding stay.
- **Author block:** figcaption is a plain column — name (15px, weight 600, snow) over city · date (`.mono` at .85rem, `--night-muted`). Per the follow-up directive the monogram discs are gone (markup, `.avatar` CSS, and the three `--avatar-*` tint tokens all removed); the order photo stays under the meta per the standing directive that product photos are not customer avatars.
- **Structure:** `.tshot` is a direct child of `.tcard` (not inside `figcaption`), spaced by `margin-top: 1.25rem` matching the figcaption rhythm.
